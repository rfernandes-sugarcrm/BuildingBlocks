...
final class SyncLinearBackoffDispatcher implements DispatcherInterface {

    /**
     * Dispatches an event with linear backoff retry logic.
     *
    ...
    public function dispatch(ExternalResourceClient $client, $ewlb, string $payload, array $headers): array
    {
        ...
        for ($attempt = 0; $attempt < $maxRetries; $attempt++) {
            ...
            sleep($initialDelay * ($attempt + 1)); // Linear backoff: 1s, 2s, 3s, ...
        }
        return [
            'attempts' => $maxRetries,
            'logs' => $logs,
